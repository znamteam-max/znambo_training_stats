...
import { addAthleteNote, getOrCreateTelegramAthlete, storeCalculatedActivity } from "./db";
...
import type { Env, StravaSummaryActivity } from "./types";
...
function buildSampleStreams(records: FitMessage[], startDate: Date) {
  ...
  return { watts, wattTimes, heartrate };
}
...
export async function importTelegramFitFile(input: {
  ...
  const athlete = await getOrCreateTelegramAthlete(
    input.env,
  ...
  const activity = await storeCalculatedActivity(input.env, {
    athlete,
  ...
  await addAthleteNote(input.env, {
    telegramChatId: input.telegramChatId,
